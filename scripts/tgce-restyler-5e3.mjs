...
const CSS_HREF              = "tgce-restyler-5e3.css";
    ...
    activateListeners(html) {
        ...
        html[0].querySelectorAll("[data-action]").forEach(element => {
            ...
            switch (element.dataset.action) {
                ...
                case "selector"         : element.addEventListener("change"     , this._onSelectorValueChange.bind(this));  break;
...
Hooks.on("renderRestyler", function(object) {   
    ...
    themeselector.selectedIndex = Array.from(themeselector.options).findIndex((e) => e?.value == game.settings.get("dnd5e","theme"))
...
//! SCRATCH BELOW
function relocateNavButtons(event){
    debugLog('green', "Inside relocateNavButtons function...")
    const myStyleSheet  = Array.from(document.styleSheets).find((e) => e?.href?.includes(CSS_HREF));   
    const navCss    = [
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `width`, `3rem`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `height`, `2rem`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `padding`, `0px`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `flex-direction`, `row`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `justify-content`, `center`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `align-items`, `center`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item`, `border-radius`, `8px`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item::after`, `inset`, `0.25rem`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item::after`, `border`, `1px solid`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs > .item::after`, `border-radius`, `4px`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs[data-group="primary"]`, `top`, `10px`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs[data-group="primary"]`, `left`, `8%`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs[data-group="primary"]`, `display`, `flex`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs[data-group="primary"]`, `flex-direction`, `row`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs[data-group="primary"]`, `gap`, `2rem`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character:not(.minimized) nav.tabs[data-group="primary"]`, `z-index`, `10`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character nav.tabs > .item.active`, `transform`, `scale(1.25)`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character nav.tabs > .item:hover`, `transform`, `scale(1.25)`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character nav.tabs`, `--tab-full-width`, `initial`],
        [`${Restyler.cssAnchorID}.dnd5e2.sheet.actor.character nav.tabs`, `--tab-inactive-width`, `initial`]
    ]
    if (event.target.checked){ 
        Array.from(navCss).forEach((e) => {  
            debugLog('green', `- Creating CSS Rule: ${e[0]} {${e[1]}: ${e[2]};}`);
            const [selector, property, value] = e;           
            createOrUpdateCssRule(selector, property, value);       
        });
    } else {
        Restyler.actor.unsetFlag(MODULE, `elements.${event.target.id}`)
        const ruleList = Array.from(myStyleSheet.cssRules).filter((r) => r.cssText.includes(Restyler.cssAnchorID));
        ruleList.forEach((r) => debugLog('red', r.cssText));
        Array.from(navCss).forEach((e) => {
            debugLog('green', e);
            const index = [...myStyleSheet.cssRules].findIndex((r) => r.cssText === e);
            debugLog('green', `- Removing CSS Rule: ${index}`);
            const [selector, property, value] = e;
            removePropertyOrRule(selector, property, value);
        })
    }
